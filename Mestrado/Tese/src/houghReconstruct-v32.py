...
def getPlanes(A1,A2,P1,P2):
	L1 = []
	L2 = []
	
	# Para cada linha, calcular os parametros do plano 3D definido pela projeccao da mesma.
	for i in A1:
		L1.append(i.dot(P1))
...
def match(A1,A2,P1,P2):
	...
	LABC = []
	# obter os planos que resultam da projecao das linhas em 3D
	L1,L2 = getPlanes(A1,A2,P1,P2)
	...
	for a in Ln1:
		for b in Ln2:
			...
			# Verificar se os planos sao paralelos
			para = np.dot(a[:3],b[:3])
			
			# Calcular angulo entre as linhas
			if(para <> 1):
				ang = np.arccos((para / (np.linalg.norm(a[:3])*np.linalg.norm(b[:3]))))
				Ang.append(ang)
		...
		LABC.append(np.array(L).reshape(2,4))
		C = []
		L = []
...
# reconstrucao
PTSLv = reconstruct(Lv,-20,2,0)
PTSLh = reconstruct(Lh,-5,2,1)
PTSLd = reconstruct(Ld,-1,1,2)

LPTv = generate3DLine(PTSLv,10)
